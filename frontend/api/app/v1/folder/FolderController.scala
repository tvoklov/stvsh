...
class FolderController @Inject() (val cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def head(id: String): Action[AnyContent] = Action.asyncF {
    val io = for {
      ...
      res <-
        if (folderExists) Sheet.count(id).map {
  ...
  def getSheets(id: String)(offset: Option[Long], limit: Option[Long], sortBy: Option[String]): Action[AnyContent] = Action.asyncF {
    val io =
      for {
        maybeFolder <- id.getFolder
        res <- maybeFolder.fold(NotFound("no folder with given id").pure[ConnectionIO]) {
          _.getSheets(offset, limit)
            .map(_.map(_.toJson))
            .map(Json.toJson(_))
            .map(Ok(_))
        }
  ...
  def post: Action[AnyContent] = Action.asyncF {
    _.body.asJson
      ...
      .fold(BadRequest("bad json value").pure[ConnectionIO])(
        _.toFolder.save.map(_.toJson).map(Ok(_))
      )
      .perform
  }
